...
ServerSocket::ServerSocket(ServerConfig config, int epollInstance)
{
	...
	if ((_socketFd = socket(config.sin_family, SOCK_STREAM, 0)) == -1) {
		close(_socketFd);
		throw SocketError("Error during socket creation. Error code : " + itoa(errno));
	}
	...
	if (bind(_socketFd, (struct sockaddr*) &serverAddress, sizeof(serverAddress)) == -1) {
		close(_socketFd);
		throw BindError("Error while binding adress to the server socket. Error code : " + itoa(errno));
	}

	// listen at the server socket and allow 5 connexions at a time
	if (listen(_socketFd, 5) == -1) {
		close(_socketFd);
		throw ListenError("Error while setting socket to listening. Error code :" + itoa(errno));
	}
