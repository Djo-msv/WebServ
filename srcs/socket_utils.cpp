#include "../includes/socket_utils.hpp"

int throw_fcntl_error(int fcntl)
{
	if (fcntl == -1)
		throw std::runtime_error("Could not perform fcntl operation error code :" + errno);
	return (fcntl);
}
...
void	epoll_add(int epollInstance, int fd, u_int32_t flags)
{
	...
	{
		close(fd);
		throw std::runtime_error("Could not add '" + itoa(fd) + "' socket to epoll interest list. Error Code :" + itoa(errno));
	}
}
